-- Physics ingestion validation/CI readiness coverage dashboard rollup.
-- Additive read-only extension of the broader validation CI readiness surface
-- over the Phase 7 coverage dashboard surface. Existing
...
source_adapter_execution_validation_rollup AS (
    SELECT
        COUNT(*) AS row_count,
...
source_adapter_execution_validation_blockers AS (
    SELECT ARRAY(
        SELECT DISTINCT blocker
        FROM (
            SELECT UNNEST(row_data.blockers) AS blocker
            ...
            UNION ALL
            SELECT 'no_source_adapter_execution_validation_readiness_data'
            WHERE (SELECT row_count FROM source_adapter_execution_validation_rollup) = 0
        ) blocker_rows
        ORDER BY blocker
    ) AS blockers
),
coverage_dashboard_rollup AS (
    SELECT
        COUNT(*) AS row_count,
...
coverage_dashboard_blockers AS (
    SELECT ARRAY(
        SELECT DISTINCT blocker
        FROM (
            SELECT UNNEST(row_data.blockers) AS blocker
            ...
            UNION ALL
            SELECT 'no_physics_ingestion_coverage_dashboard_data'
            WHERE (SELECT row_count FROM coverage_dashboard_rollup) = 0
        ) blocker_rows
        ORDER BY blocker
    ) AS blockers
),
readiness_rows AS (
    ...
    FROM source_adapter_execution_validation_rollup saavr
    CROSS JOIN source_adapter_execution_validation_blockers saevb
    UNION ALL
    SELECT
        ...
        cdb.blockers,
        jsonb_build_object(
            'snapshot_count', cdr.snapshot_count,
            ...
            'symbolic_expression_count', cdr.symbolic_expression_count,
            'blocker_count', cdr.blocker_count
        ) AS readiness_detail
    FROM coverage_dashboard_rollup cdr
    CROSS JOIN coverage_dashboard_blockers cdb
)
SELECT
    readiness_family,
    row_count,
